...
const monitorChainAbi = require('../ethereum/build/MonitorChain').abi;
const erc20 = require('../ethereum/build/TokenInterface').abi;
...
class ERC20 extends ContractFactory{
    ...
    static web3 (web3Instance, contractAddress, abi) {
        return new ERC20(null, contractAddress, null, web3Instance, abi)
    }
    ...
    async transfer(to, value, callback) {
        await this.init();
        const [err, result] = await _to(this.contract.methods.transfer(
            toChecksum(to),
            value)
            .send({
                from: this.wallet,
...
class MonitorChain extends ContractFactory {
    ...
    async getTokenInfo(tokenAddress, callback) {
        const address = toChecksum(tokenAddress);
        const token = new ERC20(this.w3, address);
        const [err, result] = await token.tokenInfo(callback);
        return returnValue(err, result, callback);
    }

    async clearErrorStatus(token, message, callback) {
        await this.init();
        let [err, result] = await _to(this.contract.methods.clearErrorStatus(
            toChecksum(token),
            message)
            .send({
                from: this.wallet,
    ...
    async addSender(sender, callback) {
        await this.init();
        let [err, result] = await _to(this.contract.methods.addSender(
            toChecksum(sender))
            .send({
                from: this.wallet,
                gas: this.gasLimit,
    ...
    async addSupportedTokens(tokenAddresses, callback) {
        await this.init();
        const chunk = 100;
        let err, result;
        for (let i = 0; i<tokenAddresses.length; i += 100)
        {
            [err, result] = await _to(this.contract.methods.addSupportedTokens(tokenAddresses.map(toChecksum).slice(i, i+chunk))
                .send({
                    from: this.wallet,
                    gas: this.gasLimit,
                    gasPrice: this.gasPrice
                }));
            if (err) return returnValue(err, result, callback);
        }
        return returnValue(err, result, callback);
    }

    async addSupportedToken(tokenAddress, callback) {
        await this.init();
        const [err, result] = await _to(this.contract.methods.addSupportedToken(
    ...
    async removeSupportedToken(tokenAddress, callback) {
        await this.init();
        const [err, result] = await _to(this.contract.methods.removeSupportedToken(
            toChecksum(tokenAddress))
            .send({
                from: this.wallet,
                gas: this.gasLimit,
